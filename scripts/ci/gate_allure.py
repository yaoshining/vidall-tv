...
def summary_html(data):
    return '<meta charset="utf-8"><h1>本次必需测试门禁：' + ('通过' if data['gate_passed'] else '失败') + '</h1><pre>' + html.escape(json.dumps({k: v for k, v in data.items() if k != 'cases'}, ensure_ascii=False, indent=2)) + '</pre>'
