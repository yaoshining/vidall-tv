...
未验证：API 19 实机、硬件 TEE 安全强度/断电重启、系统备份向导和跨物理设备恢复、真实服务器连接、恢复提示的遥控器视觉验收，以及完整远端 CI。不得把这些边界写成已通过。PR 等待独立验收，不自动合并。


## 2026-09-14 审查修复复验

上述首次交付的 21 项证据对应 `3acf53b53e602ce9d99e67dd2527dbc003f2c624`，保留为历史记录；仅删除其本地 status 中无效的 Actions 地址，其他字段不变。用户随后确认该版本在 MateTV Pro（API 24 Release）真机验证通过，未提供逐例日志；不将该确认扩大为本次修复版本的真机验证。

本次修复代码提交 `97bc09858a5e2eaf107d5e14dde81246a3ad6e3e`：

- CredentialStorage/CryptoUtil 的全部 catch 显式绑定 `e`，不改变失败处理或记录错误内容。
- 密文识别只匹配完整已知格式：v2 的三段结构、规范 Base64、12 字节 nonce 和至少 17 字节密文/标签；旧格式为规范 Base64 的 16 字节 IV 和非空、16 字节对齐密文。`v2:example`、短载荷、错误 nonce 长度等普通凭据可正常保存。
- WebDAV/SMB 首页目录入口捕获凭据读取失败时显示统一恢复提示；其他解析错误仍显示原通用错误，保留原返回/不导航行为。

应用与 ohosTest 均通过 devecocli 构建；TV API 24 模拟器真实 HUKS、真实 DAO 回归 **25/25，通过，0 failure、0 error、0 ignore**。新增四个用例分别覆盖两个 DAO 的版本前缀普通密码插入/更新/回读，以及完整新旧密文插入/更新拒绝且原值不变。现有严格门禁未修改，本次独立执行记录通过。

[本次证据](credential-migration-evidence/review-20260914/) 包含代码 SHA、源码和 HAP 摘要、逐例记录和门禁状态。后续证据/说明提交不改变已测试源码。没有更新真机应用；两条提示的遥控器视觉效果本次未实测。原主模块保留，只安装并移除隔离测试模块。
