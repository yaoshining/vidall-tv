...
自 v3.0.0 起，刮削进一步支持**多层级范围化**执行（电影 / 整剧 / 整季 / 单集 / 文件夹 / 全局），并将刮削收敛为**统一的异步任务队列**，提供全局可见的任务状态、刮削管理页、失败详情与日志查看能力，便于精确纠正误匹配的元数据。
...
### 5. 多层级范围化刮削
- 支持 **电影 / 整剧(series) / 整季(season) / 单集(episode) / 文件夹(folder) / 全局(global)** 六类刮削范围。
- 在各详情页（电影详情、剧集详情、季详情、单集操作菜单）与文件浏览器工具栏提供可聚焦入口（遥控器友好）。
- 三档处理模式：**新增和已修改**(默认) / **缺少元数据**(fix-missing) / **覆盖**(overwrite)。覆盖模式入队前保留危险操作确认。
- 候选确认策略：**自动**(automatic，自动采用最佳候选) / **确认**(confirm，局部刮削时可选多个候选之一)。默认关闭候选确认，全局刮削忽略该设置自动执行。

### 6. 异步刮削任务队列
- 有限并发的后台任务队列（默认 2，可配置 1-4）。
- 任务状态机：`waiting` → `running` → `completed` / `partial-failure` / `failed` / `cancelled`，候选确认暂停态 `waiting-confirmation`。
- 支持**排队、去重（按任务键）、等待确认（候选）、取消等待任务、重试失败、清理终态任务**。
- 任务脱离触发页面生命周期继续执行，完成后通过全局媒体数据版本刷新相关数据，不阻塞导航或播放。

### 7. 任务可视化管理
- Home **TopBar** 右上角任务图标 + 活动数量角标（仅统计进行中/等待中）。
- **刮削管理页**（`ScrapeManagementPage`）：Tab 分组「正在刮削 / 已完成 / 失败 / 已取消」。
- **失败详情页**（`ScrapeFailureDetailPage`）：展示失败项（文件名/完整路径/失败分类/详细信息）。
- **日志查看器**（`LogTextViewerPage`）：查看失败任务原始日志，支持复制。
...
除全局刮削外，可在以下上下文发起局部刮削（遥控器焦点直达）：
- **电影详情页**：刮削本片
- **剧集详情页**：刮削整部剧
- **季详情页 / 集操作菜单**：刮削整季 / 刮削本集
- **文件浏览器工具栏**：刮削当前文件夹（递归）

局部刮削同样弹出「处理模式 + 候选策略」选择；覆盖模式需确认。任务入队后在 TopBar 角标可见。

### 查看与管理任务

1. 点击 Home TopBar 右上角**任务图标**（带活动数量角标）。
2. 下拉列表查看 **进行中 / 等待中** 任务及进度环。
3. 点击「更多按钮」进入**刮削管理页**（ScrapeManagementPage），按 **正在刮削 / 已完成 / 失败 / 已取消** 分组查看。
4. 失败任务可进入 **失败详情页** 查看失败项（文件名 / 完整路径 / 失败分类 / 详细信息）。
5. 点击「查看原始日志」进入 **日志查看器**，支持复制完整日志；失败任务可「重试」。
...
#### 多层级范围化刮削与任务队列架构（v3.0.0）

```text
┌──────────────┐   ┌─────────────┐   ┌──────────────┐
│ 详情页/文件浏览器 │ → │ScrapeScopeResolver│ ← │ ScrapeModeFilter│
│ 各局部刮削入口  │   │ 范围解析/目标解析 │   │ 处理模式过滤    │
└──────┬───────┘   └──────┬──────┘   └──────┬──────┘
       │                  │                  │
       ▼                  ▼                  ▼
┌──────────────────────────────────────────────┐
│            ScrapeTaskQueue（单例队列）          │
│  waiting→running→completed/partial-failure/   │
│  failed/cancelled；并发/去重/等待确认/重试/取消    │
└──────────────────────┬───────────────────────┘
                       │ 执行
                       ▼
┌──────────────┐   ┌──────────────┐   ┌──────────────┐
│ScopedScrapeService│ → │  ScrapeClient/TmdbProvider │ → │FileSourceDatabase│
│ 统一执行/应用     │   │  候选搜索       │   │  写库          │
└──────────────┘   └──────────────┘   └──────────────┘
                       │
                       ▼
┌──────────────┐   ┌──────────────┐
│ScrapeFailureLogStore│ │MediaDataVersionStore│
│ 失败日志持久化    │   │ 全局版本/刷新通知    │
└──────────────┘   └──────────────┘
```

- **ScrapeTaskStore**：只读可观察任务状态（供 TopBar 角标 / 下拉 / 管理页派生）。
- **ScrapeTaskQueue**：调度核心，单例；并发按任务槽控制（默认 2，`PrefKey.SCRAPE_CONCURRENCY` 可配 1-4）。
- **ScopedScrapeService**：统一范围解析、候选搜索、候选应用、写库核心；失败保留已有元数据与范围外关联。
- **MediaDataVersionStore**：全局媒体数据版本，任务完成后触发相关页面刷新。
- **ScrapeFailureLogStore / LogTextViewerPage**：失败任务原始日志持久化与查看。
...
### v3.0.0 (2026-08-26)
- ✅ 实现多层级范围化刮削（电影/整剧/整季/单集/文件夹/全局）
- ✅ 实现统一异步刮削任务队列（并发/去重/等待确认/重试/取消/清理）
- ✅ TopBar 任务角标 + 下拉任务列表（进行中/等待中 + 进度环）
- ✅ 新增刮削管理页（正在刮削/已完成/失败/已取消）
- ✅ 新增失败详情页（失败项：文件名/完整路径/失败分类/详细信息）
- ✅ 新增日志查看器（原始日志查看 + 复制）
- ✅ 局部/全局刮削统一处理模式（新增和已修改/缺少元数据/覆盖）+ 候选确认策略
- ✅ 修复文件名解析：目录名括号包裹季号（如 `金色 (2026)`）导致刮削未匹配
