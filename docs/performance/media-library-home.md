...
- 使用 `devecocli build --modules entry@ohosTest` 构建真实集成测试HAP；集成工作流增加默认false的专用冒烟开关，保留原编译、执行、门禁和设备互斥。专用运行在Actions名称、Summary及产物scope.txt注明仅3项，不发布或覆盖全量集成Portal/latest，默认集成报告流程不变。
