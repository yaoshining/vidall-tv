...
- **TV 屏幕尺寸不固定**：设计稿基于 1920×1080 绘制，但实际 TV 分辨率多样（4K、1080p、720p）。所有尺寸、字号、间距必须使用**相对单位（vp）**，禁止写死 px，关键尺寸以设计稿比例换算后用 `vp` 表达
...
## EpisodeListPanel 设计规范（来源：Pencil 设计文件）

> 设计稿路径：`/Users/yaoshining/Library/Mobile Documents/com~apple~CloudDocs/Pencil/vidall_tv/vidall_pages.pen`
> 相关画框：[19] 播放器设置-选集 / [20] 播放器设置-选集-翻页态 / [21] 播放器设置-选集-分页焦点态
>
> ⚠️ **TV 屏幕尺寸不固定**：设计稿基于 1920×1080 绘制，所有尺寸必须以**比例换算为 vp**，不得写死 px 绝对值。

### Episode Drawer 整体布局

| 属性 | 设计值（1920px 基准） | 说明 |
|------|---------------------|------|
| 位置 | y=470（屏幕下方） | 占屏幕高度约 56% 以下 |
| 高度 | 610px | 约屏幕高度的 56% |
| 背景 | `#00000000`（透明）| 全屏 scrim 渐变遮罩叠加 |
| 标题 "选集" | fontSize=56, fontWeight=700, fill=`#F3F6FB` | x=84, y=82（相对 Drawer） |

### 集数卡片（Episode Card）

| 状态 | 宽 | 高 | 圆角 | 描边 |
|------|------|------|------|------|
| **当前播放**（高亮）| 210px | 136px | 26px | inside 4px `#A9F4FF` |
| 普通集 | 256px | 136px | 26px | 无 |
| 边缘半显（edgeCard） | 168px | 136px | 26px | 无 |
| 卡片间距 | 48px | — | — | — |

- card1（当前集）比普通卡稍窄（210 vs 256），通过宽度差异暗示当前播放位置
- 最右侧 edgeCard（168px）故意截断，提示用户右侧还有内容可翻页
...
| 形状 | 椭圆（Ellipse） |
| 尺寸 | 42×42px |
| 描边 | inside 5px `#FFFFFF` |
| 背景 | 透明 `#FFFFFF00` |

焦点环独立于卡片渲染，通过绝对定位叠加在当前焦点卡片右下角。

### 分页范围条（Range / Pagination Bar）

| 属性 | 值 |
|------|---|
| 位置 | y=410（相对 Drawer），x=84 |
| 宽度 | 1200px |
| 高度 | 34px |
| 条目间距 | gap=58px |
| 条目内容 | "1-6", "7-12", "13-18"... （每页6集） |
| 字号 | fontSize=22, fontWeight=normal |
| 激活项颜色 | `#F3F7FB`（亮白） |
| 非激活项颜色 | `#D5DDE8`（灰） |
| 下划线 | 高3px, 圆角2px, fill=`#F5F8FC`, 宽度=84px（当前激活项宽度） |

### 翻页指示器胶囊（pagerPill，[21] 分页焦点态）
