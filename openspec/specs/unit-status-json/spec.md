...
定义单元测试的权威门禁状态在报告中的展示契约。
本规范将编译状态与设备测试执行结果分别记录，并确保必需检查、报告与历史记录基于同一次运行证据保持一致，防止未执行、执行异常或缺少结果时产生虚假的成功结论。

## Requirements

### Requirement: unit-status-json-schema
`unit-test.yml` SHALL 使用本次门禁判定结果生成 `unit-status.json`。至少包含 `suite`、`status`（`passed` / `failed` / `not_run`）、`gate_passed`、`build_status`、`reason`、`passed`、`failed`、`errors`、`total`、运行标识、commit SHA 及时间戳。
