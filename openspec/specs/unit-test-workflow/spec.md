...
### Requirement: 明确指定的 TCP 设备须先建立连接再判断可用性
workflow SHALL 在设备列表缺少指定 TCP 目标时，有界执行一次定向 `tconn`，随后重新查询并确认目标存在；不得将连接命令返回 0 单独视为设备可用。

#### Scenario: 目标可连接但尚未注册
- **WHEN** 初始列表无指定目标，定向连接成功且再次查询发现目标
- **THEN** 继续执行必需测试，仍须依据真实用例结果判定通过

#### Scenario: 连接失败或无目标
- **WHEN** 连接失败、超时，或重新查询仍找不到目标
- **THEN** 测试未执行且门禁失败，保留连接日志，不重启共享 hdc
