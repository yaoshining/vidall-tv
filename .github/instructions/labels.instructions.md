...
```
# 高优先级播放器 Bug（应立即处理）
bug + priority: high + component: player + repro: high

# 中等规模媒体库改进（按计划处理）
enhancement + priority: medium + size: medium + component: media-library + phase-2-enhanced
...
| 错误 | 正确做法 |
|------|----------|
| 只加 `bug` 标签，没有组件/优先级 | 必须同时添加 `component:*` + `priority:*` + `repro:*`（如适用） |
