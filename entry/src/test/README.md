...
- 中段全拼从音节或英文词起点匹配，例如 `senlin` 命中重庆森林；不允许从 `chong` 内部的 `ong` 起点召回。单字母全拼只做精确匹配；首字母从单字母起支持片名前缀，不做任意中段、跳字或编辑距离模糊匹配。
...
输入停止 250ms 后，从本地搜索的最多 200 项结果中按原顺序提取最多 20 个去重片名；单首字母可匹配片名前缀。键盘下横向展示候选并自动搜索第一项，保留输入与候选列表。点击或遥控器确认候选时写入片名历史，自动选中首项也写入片名历史，重复片名更新已有记录；空输入显示历史。候选选中用浅色边框、焦点用蓝色，首行结果向上回到所选候选。

```sh
TYPESCRIPT_PATH=/Applications/DevEco-Studio.app/Contents/tools/hvigor/hvigor/node_modules/typescript node entry/src/test/search_suggestions_test.cjs
TYPESCRIPT_PATH=/Applications/DevEco-Studio.app/Contents/tools/hvigor/hvigor/node_modules/typescript node entry/src/test/search_scope_test.cjs --integration --server-search
```

主机测试覆盖两阶段查询、原输入保留、点击历史、空结果、失败重试和输入/清空/来源变化的迟到响应。TV 实机仍需检查：键盘下行到候选、横向滚动可见性、确认切换结果、结果上行返回候选、清空后历史与删除。


紧凑布局新增主机焦点检查：所有键盘底行 DOWN → 选中候选，候选 UP → 原键、DOWN → 结果，结果 UP → 候选/历史，以及来源切换、空列表和删除后的有效焦点回退。另检查三组结果视口下六列海报及标题的尺寸边界。页面改用固定操作区和独立结果滚动；实际 TV 字号、长标题 MARQUEE 和滚动可见性仍需原生界面验收。
