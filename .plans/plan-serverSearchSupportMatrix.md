...
2026-09-08，基于 `aad1dfaa0` 修复。本文记录当前实现与验证边界，不代表 Issue 已满足全部完成定义。

### 输入能力

| 来源 | 输入 | 提交 | 历史 |
| --- | --- | --- | --- |
| 本地文件源 | 现有字母/数字屏幕键盘，保留拼音检索；共享 TextInput | 防抖或 IME 提交；显式提交取消待执行防抖 | 仅本地读写 |
| Jellyfin / Emby / Plex | 共享真实 TextInput，隐藏本地屏幕键盘；点击/OK 进入系统编辑 | 搜索按钮或 IME 提交，结束编辑后搜索；不依赖服务器拼音 | 不读写本地历史 |
| 不可用来源 | 隐藏输入及结果 | 不搜索、不回退其他来源 | 不显示旧历史 |

`enableKeyboardOnFocus(false)` 遵循 TV 输入习惯：方向键或程序获焦不自动弹出键盘；点击输入框仍可编辑。依据本地 HarmonyOS TextInput 文档，实际遥控器 OK 和系统输入法行为仍需设备验证。

### 来源与状态契约

- 本地与服务器、服务器 A/B、有效与不可用来源之间切换：清空关键词、结果、历史视图，取消防抖并使旧响应失效。必须在新来源重新输入，不能自动转发上个来源的词。
- 页面隐藏期间切源也按上述契约处理；同来源从详情返回仍保留词并重新搜索，重复 onShown 不重复安排请求。
- 显式路由携带其他来源时丢弃路由关键词。没有来源的旧路由视为当前来源输入，保持兼容。
- 清空：结束编辑、清空词与结果、取消待执行搜索，返回输入区。
- 取消编辑 / 硬件返回：编辑态先结束编辑并保留当前来源输入；结果卡片区返回输入区；输入区再次返回退出。页面左上返回按钮直接退出。
- 页面显示时请求输入区焦点，不自动弹出键盘；使用方向键自然进入结果区、返回键回到输入区，不额外提供焦点跳转按钮；所有结果卡片共用焦点样式和命名规则。

### 共享结果框架

两种来源经过 `resultItems()` 归一为 `WorkspaceSearchItem`，统一使用 `buildSearchResults()`、`buildResultList()`、`WorkspacePosterCard` 与 `openResult()`。加载/错误/空态、列表布局、焦点样式和选择入口均共享，不再保留服务器独立卡片或列表。

差异限于内容与来源适配：服务器保留类型标签、首批最多 100 项提示和服务器身份快照校验；本地保留评分、本地海报和电影/剧集详情路由。旧来源卡片不能通过统一入口导航到详情。归一对象只存在于内存，不写日志或持久化，服务器图片地址继续沿用原有鉴权方式。

### 已执行验证

- `devecocli build --modules entry@default --product default`：生产构建成功，有工程及依赖警告。
- `search_scope_test.cjs --server-search --integration`：59 项 Hypium 主机测试通过，并通过真实页面方法的集成/连续链回归。
- 新增隔离与统一选择/焦点连续链 19 个断言；更新旧路由与隐藏期间切源连续链 14 个断言。覆盖本地→A→B→本地、切源后迟到响应、旧本地卡片拒绝、两类详情适配、结束编辑及返回焦点目标。
- 传输、数据库、计时器与 ArkUI 焦点控制为边界替身；主机通过不能代替实际 TV/HTTP 验收。
...
1. TV 上以 D-Pad/OK 聚焦输入、打开系统键盘、中文输入、取消和收起；确认输入区与结果区往返以及详情返回没有焦点丢失。
2. 至少一个真实服务器实例与本地 A/B 切换，验证关键词不自动跨源、真实搜索/详情/返回闭环；记录设备、系统、服务器版本及待测提交，不记录凭据。
3. 获得至少一名评审通过，完成 PR 合并。完成前不将 #317 标为 DONE。

以下 3.1 / 3.2 保留历史验证记录；其中旧结果分支和跨来源关键词保留描述已由上面的当前矩阵替代。
